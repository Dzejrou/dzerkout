...
async fn test_create_exercise_with_catalog_meta_and_muscles(pool: SqlitePool) {
    ...
    let ex = exercise::create(&pool, "Barbell Bench Press", None, &[], Some(&m), Some(&ms))
        .await
        .unwrap();
...
async fn test_update_exercise_replaces_muscles(pool: SqlitePool) {
    let ex = exercise::create(
        &pool, "Squat", None, &[],
        None,
        Some(&muscles(&[("quadriceps", "primary"), ("glutes", "secondary")])),
    )
    .await
    .unwrap();
...
async fn test_update_exercise_no_muscles_preserves_existing(pool: SqlitePool) {
    let ex = exercise::create(
        &pool, "Deadlift", None, &[],
        None,
        Some(&muscles(&[("hamstrings", "primary"), ("glutes", "secondary")])),
    )
    .await
    .unwrap();
...
async fn test_invalid_category_rejected(pool: SqlitePool) {
    ...
    let result = exercise::create(&pool, "Handstand", None, &[], Some(&m), None).await;
    assert!(
...
async fn test_invalid_equipment_rejected(pool: SqlitePool) {
    ...
    let result = exercise::create(&pool, "Jump", None, &[], Some(&m), None).await;
...
async fn test_invalid_level_rejected(pool: SqlitePool) {
    ...
    let result = exercise::create(&pool, "Planche", None, &[], Some(&m), None).await;
    assert!(matches!(result, Err(AppError::Validation(_))));
}
...
async fn test_invalid_mechanic_rejected(pool: SqlitePool) {
    ...
    let result = exercise::create(&pool, "Box Jump", None, &[], Some(&m), None).await;
    assert!(matches!(result, Err(AppError::Validation(_))));
}
...
async fn test_invalid_force_rejected(pool: SqlitePool) {
    ...
    let result = exercise::create(&pool, "Power Clean", None, &[], Some(&m), None).await;
    assert!(matches!(result, Err(AppError::Validation(_))));
}
...
async fn test_invalid_muscle_name_rejected(pool: SqlitePool) {
    ...
    let result = exercise::create(&pool, "Press", None, &[], None, Some(&ms)).await;
    assert!(matches!(result, Err(AppError::Validation(_))));
}
...
async fn test_invalid_muscle_role_rejected(pool: SqlitePool) {
    ...
    let result = exercise::create(&pool, "Fly", None, &[], None, Some(&ms)).await;
    assert!(matches!(result, Err(AppError::Validation(_))));
}
...
async fn test_invalid_instructions_json_not_json(pool: SqlitePool) {
    ...
    let result = exercise::create(&pool, "Run", None, &[], Some(&m), None).await;
    assert!(matches!(result, Err(AppError::Validation(_))));
}
...
async fn test_delete_cascades_exercise_muscles(pool: SqlitePool) {
    let ex = exercise::create(
        &pool, "Curl", None, &[],
        None,
        Some(&muscles(&[("biceps", "primary"), ("forearms", "secondary")])),
    )
    .await
    .unwrap();
...
async fn test_duplicate_catalog_id_rejected(pool: SqlitePool) {
    ...
    exercise::create(&pool, "Squat", None, &[], Some(&m), None)
        .await
        .unwrap();
    ...
    let result = exercise::create(&pool, "Squat Variant", None, &[], Some(&m2), None).await;
    assert!(
...
async fn test_plain_exercise_create_still_works(pool: SqlitePool) {
    let ex = exercise::create(&pool, "Pull-up", Some("dead hang start"), &["pull".into()], None, None)
        .await
        .unwrap();
...
async fn test_valid_instructions_json_accepted(pool: SqlitePool) {
    ...
    let ex = exercise::create(&pool, "Romanian Deadlift", None, &[], Some(&m), None)
        .await
        .unwrap();
...
async fn test_empty_instructions_json_array_accepted(pool: SqlitePool) {
    ...
    let ex = exercise::create(&pool, "Side Bridge", None, &[], Some(&m), None)
        .await
        .unwrap();
